...
func TestTrie_InsertManyTimes(t *testing.T) {
	...
	for r := 0; r < 20; r++ {
		...
		got = trie.RootHash()
		require.Equal(t, expectedRootHashes[r], hex.EncodeToString(got[:]), "Iteration %d", r)
	}
...
func TestTrie_InsertDeallocateRegisters(t *testing.T) {
	...
	// Now we override the first 99 registers with nil values, i.e. deallocate them.
	payloads0 := make([]ledger.Payload, len(payloads1))
	testTrie, err = testTrie.Mutate(paths1, payloads0)
	require.NoError(t, err)

	// This should be identical to having never written the first 99 registers in the first place.
...
func BenchmarkTrie_InsertEmptyTrie(b *testing.B) {
	...
	for i := 1; i <= 32768; i *= 2 {
		...
		b.Run(fmt.Sprintf("insert %d elements into new trie (ref)", i), func(b *testing.B) {
			for j := 0; j < b.N; j++ {
				b.StopTimer()
		...
		b.Run(fmt.Sprintf("insert %d elements into new trie (new)", i), func(b *testing.B) {
			for j := 0; j < b.N; j++ {
				b.StopTimer()
				paths, payloads := helpers.SampleRandomRegisterWrites(helpers.NewGenerator(), i)
				b.StartTimer()
...
func BenchmarkTrie_InsertFullTrie(b *testing.B) {

	for i := 1024; i <= 32768; i *= 2 {

		b.Run(fmt.Sprintf("insert %d elements into full trie (ref)", i), func(b *testing.B) {
			ref := reference.NewEmptyMTrie()
			paths, payloads := helpers.SampleRandomRegisterWrites(helpers.NewGenerator(), 50_000_000)
			for i := range paths {
				ref, _ = reference.NewTrieWithUpdatedRegisters(ref, paths[i:i], payloads[i:i])
			}
			_ = ref.RootHash()
			b.ResetTimer()
			for j := 0; j < b.N; j++ {
		...
		b.Run(fmt.Sprintf("insert %d elements into full trie (new)", i), func(b *testing.B) {
			tr := trie.NewEmptyTrie()
			paths, payloads := helpers.SampleRandomRegisterWrites(helpers.NewGenerator(), 50_000_000)
			for i := range paths {
				tr, _ = tr.Mutate(paths[i:i], payloads[i:i])
			}
			_ = tr.RootHash()
			b.ResetTimer()
			for j := 0; j < b.N; j++ {
